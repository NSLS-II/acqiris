 ...
 ******************************************************************************
 acqiris_init_record(): parse link fields to get module numbers, channels, and
 call acqiris_init_record_specialized() which is implemented in acqiris_drv_*
 acqiris_read_record(): just directly call acqiris_read_record_specialized()
...
static int
acqiris_bad_field(void* record, const char* message, const char* fieldname)
{
...
template<class T>
    ...
    acqiris_init_record(T* record, DBLINK link)
    {
        ...
        int status;
        status = sscanf(sinp, "M%u C%u %s", &arc->module, &arc->channel,
                arc->name);
        if (status != 3)
        {
...
template int
acqiris_init_record(longinRecord*, DBLINK);
template int
acqiris_init_record(longoutRecord*, DBLINK);
template int
acqiris_init_record(aiRecord*, DBLINK);
template int
acqiris_init_record(aoRecord*, DBLINK);
template int
acqiris_init_record(mbboRecord*, DBLINK);
template int
acqiris_init_record(waveformRecord*, DBLINK);
template int
acqiris_read_record(longinRecord*);
template int
acqiris_read_record(longoutRecord*);
template int
acqiris_read_record(aiRecord*);
template int
acqiris_read_record(aoRecord*);
template int
acqiris_read_record(mbboRecord*);
template int
acqiris_read_record(waveformRecord*);
template int
acqiris_write_record(longoutRecord*);
template int
acqiris_write_record(aoRecord*);
template int
acqiris_write_record(mbboRecord*);
