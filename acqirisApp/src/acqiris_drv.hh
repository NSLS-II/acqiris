...
//max. number of channels of that board(no interleaving)
  int nchannels;
//effective channels when combining channels(interleaving), see acqiris_drv_lo.cp
  int effectiveChs;
  int id;
  IOSCANPVT ioscanpvt;
  ...
  unsigned readerrors;
  unsigned truncated;
  acqiris_data_t data[MAX_CHANNEL];//see AcqrsD1_readData() in acqiris_daq.cpp
...
//global variable: array of acqiris driver structure: each element for one card
