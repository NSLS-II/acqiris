...
static int
rai_MDelayOffset(rec_t* rec, ad_t* ad, double* val)
{
    char s[256];
...
static int
rai_MDelayScale(rec_t* rec, ad_t* ad, double* val)
{
    char s[256];
...
static int
rai_MExtCkRatio(rec_t* rec, ad_t* ad, double* val)
{
    char s[256];
...
static int
rai_MSSRTimeStamp(rec_t* rec, ad_t* ad, double* val)
{
    char s[256];
...
static int
rai_CTrigLevelRange(rec_t* rec, ad_t* ad, double* val)
{
    char s[256];
...
typedef int
(*acqiris_parai_rfunc)(rec_t* rec, ad_t* ad, double* val);
...
template<>
    int
    acqiris_init_record_specialized(aiRecord* pai)
    {
...
template<>
    int
    acqiris_read_record_specialized(aiRecord* pai)
    {
        acqiris_record_t* arc = reinterpret_cast<acqiris_record_t*> (pai->dpvt);
        ad_t* ad = &acqiris_drivers[arc->module];
        acqiris_airecord_t* rai =
                reinterpret_cast<acqiris_airecord_t*> (arc->pvt);
